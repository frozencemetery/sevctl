...
fn main() {
    ...
    let matches = App::new("SEV Platform Control")
        ...
        .subcommand(
            SubCommand::with_name("reset").about("Resets the SEV platform"),
        )
        .subcommand(
        ...
        .subcommand(
            SubCommand::with_name("serve")
                .about(
                    "Run a server to handle OCA certificate signing requests",
                )
                .arg(
                    Arg::with_name("cert")
    ...
    fn sev_chain(filename: Option<&str>) -> sev::Chain {
        match filename {
            ...
            Some(f) => {
                ...
                sev::Chain::decode(&mut file, ())
                    .expect("unable to decode chain")
            }
        }
    }
...
mod generate {
    ...
    pub fn cmd(matches: &ArgMatches) -> ! {
        ...
        let mut crt =
            File::create(crt).expect("unable to create certificate file");
        oca.encode(&mut crt, ())
...
mod serve {
    ...
    fn load(matches: &ArgMatches) -> (Vec<u8>, PrivateKey<sev::Usage>) {
        ...
        // Load certificate
        let mut oca =
            File::open(oca).expect("unable to open certificate file");
        ...
        let key = PrivateKey::decode(&mut key, &oca)
    ...
    fn sign(
        ...
        let mut pek =
            sev::Certificate::decode(&mut &buf[..], ()).or(Err(400u16))?;
